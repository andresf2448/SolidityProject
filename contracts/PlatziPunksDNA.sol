...
contract PlatziPunksDNA {
    ...
    // Esta funcion es determinista y no puede ser usada en producion
    // chain.link para produccion
    ...
    function deterministicPseudoRandomDNA (uint256 _tokenId, address _minter)
        ...
        returns(uint256)
        {
            // normalmente las direcciones publicas de las wallet tienen 40 digitos 
            // exadecimales que equivale a uint160
            uint256 combinedParams = _tokenId + uint160 (_minter);
            
            // con el abi.encodePacked convertimos un numero decimal a bytes
            bytes memory endocodeParams = abi.encodePacked(combinedParams);

            //kecak256 recibe bytes y genera un hash de 256 
            bytes32 hashedParams = keccak256(endocodeParams);
